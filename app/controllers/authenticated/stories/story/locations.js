...
export default class StoriesStoryLocationsController extends Controller {
	@service router;
	@action addLocation() {
		const createdLocation = this.store.createRecord("location", {story: this.story});
	...
	@action removeLocation(location) {
		return location.destroyRecord().then(() => this.router.transitionTo("stories.story.locations"));
	}
}
